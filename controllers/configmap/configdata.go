...
AERO_CLUSTER_JSON="$(curl -f --cacert $CA_CERT -H "Authorization: Bearer $TOKEN" "$KUBE_API_SERVER/apis/aerospike.aerospike.com/v1alpha1/namespaces/$NAMESPACE/aerospikeclusters/$AERO_CLUSTER_NAME")"
...
cat /tmp/patch.json | curl -f -X PATCH -d @- --cacert $CA_CERT -H "Authorization: Bearer $TOKEN"\
     ...
     "$KUBE_API_SERVER/apis/aerospike.aerospike.com/v1alpha1/namespaces/$NAMESPACE/aerospikeclusters/$AERO_CLUSTER_NAME/status?fieldManager=pod"
`
...
func getBaseConfData(aeroCluster *aerospikev1alpha1.AerospikeCluster, rack aerospikev1alpha1.Rack) (map[string]string, error) {
	...
	workDir := aerospikev1alpha1.GetWorkDirectory(config)
	...
	initializeTemplateInput := initializeTemplateInput{
		...
		PodPort:         aerospikev1alpha1.ServicePort,
		PodTLSPort:      aerospikev1alpha1.ServiceTLSPort,
	}
